...
local Moduledir = "/etc/supervisor"
local Angelltable = {}
...
function proc_status(Args)
    ...
    do
        ...
        if not pid then            
            ...
            obj["PID"]= pid
            local rss, err = saker.sysinfo("system.proc.mem.rss", pid, "mb")
            if rss == nil then
                saker.log(LOG_ERROR, "call system.proc.mem.rss failed")
                rss = 0
            end
            obj["RSS"] = tostring(rss).."m"
            obj["PMEM"]= string.format("%.2f",infotb["PMEM"]);
            obj["PCPU"]= string.format("%.2f",infotb["PCPU"]);
            obj["UPTIME"] = "unknown"
            obj["Threads"] = infotb["Threads"]
...
function sv_config(Args)
    for _,sname in pairs(Args)
    do
        return true, json.encode(Angelltable[sname])
    end
    local rsp = {}
    rsp['code'] = 0
    return false, json.encode(rsp)
...
saker.register("svconfig",   "sv_config",           PROP_PASSIVITY)
...
saker.register("svinfo",     "proc_info",           PROP_PASSIVITY)
